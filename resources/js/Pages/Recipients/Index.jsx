...
export default function Index({ auth, recipients }) {
    ...
    const handleUpdate = (e) => {
        e.preventDefault();
        editForm.put(route('recipients.update', selectedItem.id), {
            onSuccess: () => setIsEditModalOpen(false),
        });
    };

    const handleDelete = () => {
        editForm.delete(route('recipients.destroy', selectedItem.id), {
    ...
    return (
        <SupabaseLayout user={auth.user} header="Master Tujuan">
            <Head title="Master Tujuan" />

            <div className="flex flex-col space-y-6">
                <div className="flex items-center justify-between">
                    <div>
                        <h2 className="text-xl font-bold text-slate-900 tracking-tight">Master Tujuan / Penerima</h2>
                        <p className="text-sm text-slate-500 mt-1">Kelola daftar tujuan surat per bidang untuk kemudahan registrasi.</p>
                    </div>
                    <button 
                ...
                <SupabaseTable 
                    columns={columns} 
                    data={recipients} 
                    searchPlaceholder="Cari nama tujuan..." 
                />
            </div>
