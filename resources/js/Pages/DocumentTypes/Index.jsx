...
export default function Index({ auth, documentTypes }) {
    ...
    const handleUpdate = (e) => {
        e.preventDefault();
        editForm.put(route('document-types.update', selectedItem.id), {
            onSuccess: () => setIsEditModalOpen(false),
        });
    };

    const handleDelete = () => {
        editForm.delete(route('document-types.destroy', selectedItem.id), {
    ...
    return (
        <SupabaseLayout user={auth.user} header="Jenis Dokumen">
            <Head title="Jenis Dokumen" />

            <div className="flex flex-col space-y-6">
                <div className="flex items-center justify-between">
                    <div>
                        <h2 className="text-xl font-bold text-slate-900 tracking-tight">Master Jenis Dokumen</h2>
                        <p className="text-sm text-slate-500 mt-1">Definisikan kategori dokumen untuk setiap bidang.</p>
                    </div>
                    <button 
